...
class TranslationEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # embed.shape:[batch_size,seq_len,hidden_size]

        lengths = (x != self.embedding.padding_idx).sum(dim=1)
        last_hidden_state = output[torch.arange(0, output.shape[0]), lengths - 1, :]
...
class TranslationDecoder(nn.Module):
    ...
    def forward(self, x, hidden_0, encoder_outputs):
        ...
        # hidden_0.shape:[1,batch,hidden_size]
        embed = self.embedding(x)
        ...
        # context_vector.shape:[batch_size,1,seq_len]

        # 融合信息
        combined = torch.cat((output, context_vector), dim=-1)
        # combined.shape:[batch_size,1,seq_len * 2]
