...
def train_one_epoch(model, dataloader, loss_fn, optimizer, device):
    ...
    for inputs, targets in tqdm(dataloader, desc="训练"):
        ...
        targets = targets.to(device)  # targets.shape:[batch_size,tar_seq_len]
        decoder_inputs = targets[:, :-1]  # decoder_inputs.shape:[batch_size,seq_len]
        decoder_targets = targets[:, 1:]  # decoder_targets:[batch_size,seq_len]
        # 前向传播
        # 编码阶段
        ...
        for i in range(seq_len):
            decoder_input = decoder_inputs[:, i].unsqueeze(1)  # decoder_inputs.shape:[batch_size,1]
        ...
        # decoder_outputs.shape:[tensor([batch_size,1,vocab_size])] -> [batch_size*seq_len,vocab_size]
...
def train():
    ...
    # 7.TransBoard Writer
    writer = SummaryWriter(log_dir=config.LOGS_DIR / datetime.now().strftime("%Y%m%d-%H%M%S"))
