...
class BaseTokenizer(object):
    ...
    def build_vocab(cls, sentences, vocab_path):
        ...
        vocab_list = [cls.pad_token, cls.unk_token, cls.sos_token, cls.eos_token] + [token for token in list(vocab_set)
                                                                                     if token != ' ']
...
if __name__ == '__main__':
    tokenizer = TreebankWordTokenizer()
    detokenizer = TreebankWordDetokenizer()
    word_list = tokenizer.tokenize("hajimi o na bei lu duo")
    print(word_list)
