...
def predict_batch(model, inputs, en_tokenizer):
    ...
    with torch.no_grad():
        ...
        decoder_inputs = torch.full([batch_size, 1], en_tokenizer.sos_token_index, device=device)
        ...
        for i in range(config.MAX_SEQ_LENGTH):
            ...
            decoder_output = model.decode(decoder_inputs, memory, tgt_mask, src_pad_mask)
            # decoder_output.shape:[batch_size,tgt_seq_len,en_vocab_size]

            # 保存预测结果
            next_token_indexes = torch.argmax(decoder_output[:,-1,:], dim=-1).unsqueeze(-1)
            # next_token_indexes.shape:[batch_size,1]
            generated.append(next_token_indexes)

            # 更新输入(decoder_input)
            decoder_inputs = torch.cat([decoder_inputs, next_token_indexes], dim=-1)
