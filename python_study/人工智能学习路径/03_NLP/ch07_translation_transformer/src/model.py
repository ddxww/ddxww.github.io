...
#
#         pos = torch.arange(0,max_len,dtype=torch.float).unsqueeze(1) # pos.shape:(max_len,1)
#         _2i = self.torch.arange(0,d_model,step=2,dtype=torch.float) # # _2i.shape:(d_model/2,)
#         div_term = torch.pow(10000,_2i/self.d_model)
#
#         sins = torch.sin(div_term) # sins.shape(max_len,d_model/2)
#         coss = torch.cos(div_term) # coss.shape(max_len,d_model/2)
#
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        # part_pe.shape:[batch_size,seq_len,dim_model]
        return x + part_pe
