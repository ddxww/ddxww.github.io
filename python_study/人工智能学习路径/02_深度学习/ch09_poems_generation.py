...
id_seqs,id2word,word2id = preprocess('poems.txt')
...
# 5.生成新诗（测试）
def generate_poems(model,id2word,word2id,start_token,line_num=4,line_length=7):
    model.eval()
    poem=[] #记录生成结果
    ...
    if start_id != word2id["<UNK>"]:
        ...
        current_rest_len-=1
    # 5.2定义输入数据
    input=LongTensor([[start_id]]).to(device)
    # 5.3迭代生成数据
    with torch.no_grad():
        # 按行生成诗句
        for i in range(line_num):
            # 每行生成两句诗
            for interpunctuation in ['，','。\n']:
                # 逐字生成诗句
                ...
                # 本句生成结束，添加标点符号
                poem.append(interpunctuation)
                current_rest_len=line_length

    return "".join(poem)

for i in range(10):
    print(generate_poems(model,id2word,word2id,start_token="一",line_num=4,line_length=7))